# CTRPv2 10 to 20 sample response on Exacloud
# from ctrpv2_10_percent.py on exa
# Purpose of notebook is to write less than 20k result files
# (post rm -r on CTRPv2_ten_percent_exa)

# make new shell script
    # drug index range in SBATCH array
    
# put data/coding_genes_CCLE.tsv for final exa script - done

print('2022-07-22 standard output check')
import argparse
...
date = '2022-07-22' # script created
...
raw_feats = pd.read_csv(
    'data/coding_genes_CCLE.tsv', # exa, production
    # 'data/raw_feat_1000.tsv', # Local DEVEL  <-------------- check
    sep = '\t', index_col = 0
)
# print('read time')
...
for smp_sz in list(range(50,len(drug_sub),50)): # sample step sizes, auto len, for production  
    sel_feat_dct = {}
    tmp_cols = ['rs','splt_0', 'splt_1', 'splt_2', 'splt_3', 'splt_4', 'splt_5', 'splt_6', 'splt_7', 'splt_8', 'splt_9']
    template = pd.DataFrame(columns = tmp_cols)
...
#                 round(time.time() - loop_strt,1))+'_.tsv', sep = '\t')
        ...
        predict_out.insert(0, 'rs', rs)
        template = pd.concat([template, predict_out], axis = 0)
        # break # Resampling loop 1 of 30
        
        sel_feat_dct[rs_name] = sel_feat_lst
    template.to_csv('results/CTRPv2_twenty_percent_exa/prd_'+str(
                drg)+'_'+str(
                smp_sz).zfill(3)+'_'+str( # smp_sz is the sample size,
    ...
    feats_out.to_csv('results/CTRPv2_twenty_percent_exa/fts_'+str(
                drg)+'_'+str(
                smp_sz).zfill(3)+'_'+str( # sm is the sample size, rs is resample
